...
While it is possible to use the `id` attribute as the target of a an `href` attribute this can have several undesirable side-effects; namely a global variable in Javascript (which may or may not be what we want) and possible conflicts with ID value generated by Jekyll. Therefore it is recommend to use the `name` attribute when creating named anchors.  Many browsers will also choke if the `<a>` tag does not have a closing `</a>` tag so avoid self closing tags.
...
Arbitrary YAML statements can be included in the front matter and will be available in the `page` object in the template or markup file. This useful to define data at the page level that will be used in the layout template; e.g. the navigation buttons.

The `_config.yml` can also include arbitrary YAML statements that will be accessible through the `site` object.  This is how the *Back to the top* links are generated.  See [below](#back-to-the-top).

Arbitrary YAML files can also be included in the `_data` directory and accessed through `site.data.filename`.  For example, assume `_data/people.yml` contains the following:
...
{% comment %}
See http://stackoverflow.com/questions/5319754/cross-reference-named-anchor-in-markdown/7335259#7335259
{% endcomment %}
